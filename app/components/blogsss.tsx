      ...
      {articles.map((article) => (
        <div key={article.id} className="flex flex-col justify-center items-center sm:mx-1 my-4 mx-8 p-3 border border-gray-300 rounded-[2rem]">
          <div className={`p-4 border border-gray-300 w-full h-[12rem] flex flex-col justify-end rounded-t-[2rem] ${article.blog}`}>
            <Link to={article.blog} className="w-[6rem] sm:w-[6rem] text-[0.6rem] flex bg-white hover:bg-blue-700 text-gray-700 font-medium font2 py-3 sm:py-2 justify-center items-center rounded-r-lg rounded-tl-xl sm:rounded-r-md sm:rounded-tl-lg">
              {article.buttonLabel}
            </Link>
          </div>
